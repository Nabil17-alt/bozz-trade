...
function computeATR(highs, lows, closes, period, changeATR) {
    ...
    const atr2 = ti.SMA.calculate({ period, values: tr });
    const atr2Full = Array(tr.length - atr2.length).fill(null).concat(atr2);
    ...
    return changeATR ? atrRealFull : atr2Full;
}
...
//         if (openTrade) {
//             if (
//                 (openTrade.side === "buy" && s.sellSignal) ||
//                 (openTrade.side === "sell" && s.buySignal)
//             ) {
//                 openTrade.closePrice = s.price;
//                 openTrade.result = openTrade.side === "buy"
//                     ? s.price - openTrade.openPrice
//                     : openTrade.openPrice - s.price;
//                 trades.push(openTrade);
//                 openTrade = null;
//             } else {
//                 const atr = atrArr[i] || 0;
//                 if (openTrade.side === "buy" && s.price >= openTrade.openPrice + atr * tpMultiplier) {
//                     openTrade.closePrice = s.price;
//                     openTrade.result = s.price - openTrade.openPrice;
//                     trades.push(openTrade);
//                     openTrade = null;
//                 } else if (openTrade.side === "sell" && s.price <= openTrade.openPrice - atr * tpMultiplier) {
//                     openTrade.closePrice = s.price;
...
//         if (!openTrade) {
//             if (s.sniperBuy) {
//                 openTrade = { time: s.time, side: "buy", openPrice: s.price };
//             } else if (s.sniperSell) {
//                 openTrade = { time: s.time, side: "sell", openPrice: s.price };
//             }
//         }
//     }
//     return trades;
...
function simulateTrades(signals, atrArr, tpMultiplier = 1) {
    ...
    for (let i = 0; i < signals.length; i++) {
        ...
        // Tutup posisi jika kondisi tercapai
        if (openTrade) {
            const atr = atrArr[i] || 0;
            // Close jika profit target tercapai
            if (openTrade.side === "buy" && s.price >= openTrade.openPrice + atr * tpMultiplier) {
                openTrade.closePrice = s.price;
                openTrade.result = s.price - openTrade.openPrice;
                trades.push(openTrade);
            ...
            // Close jika sinyal berlawanan muncul
            else if (openTrade.side === "buy" && s.sniperSell) {
                openTrade.closePrice = s.price;
                openTrade.result = s.price - openTrade.openPrice;
                trades.push(openTrade);
                openTrade = null;
            } else if (openTrade.side === "sell" && s.sniperBuy) {
                openTrade.closePrice = s.price;
                openTrade.result = openTrade.openPrice - s.price;
                trades.push(openTrade);
                openTrade = null;
        ...
        // Buka posisi baru jika belum ada open trade
        if (!openTrade) {
            if (s.sniperBuy) {
                openTrade = { time: s.time, side: "buy", openPrice: s.price };
            } else if (s.sniperSell) {
